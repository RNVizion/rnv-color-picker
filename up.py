...
For rnv-color-picker, derived against a fresh clone at the live head (a826a19).
...
lower-case rule. translucent() wrote #ED1A1A1A; it writes #ed1a1a1a now, whatever case
it is given. The three image overlays and the image menu ground are the
values that change spelling. Qt reads either case, so no pixel moves.
"""
...
LEFT_ALONE = ["hex the picker SHOWS rather than writes: the screen picker's readout and the settings panel's label print picked colours as #FFAA00. That is display text for a person, not a value in source.", 'sentences that name an upper-case value as history, such as the 2026-08-29 wiring pass that never matched #ED000000. They record what was written then.', "OVERLAY_BLACK_LIGHT, _MEDIUM and _HEAVY, which are integer tuples. Tuples have no case; they are the tuple round's."]

HELPER_FILE = 'utils/config.py'
HELPER = 'translucent'
HELPER_NAMES = ('_hex6', '_alpha_byte', 'translucent')
