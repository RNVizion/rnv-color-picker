"""suppress hypothesis too_slow on the history max-size property test

    python up.py             # apply, then run the test file and CI's two commands
    python up.py --check     # rehearse the edit in memory, write nothing
    python up.py --verify    # run the test file and CI's commands, change nothing

For rnv-color-picker, derived against a fresh clone at the live head (b968519).
...
RULED 2026-09-25 (ruling 6). tests/test_color_history.py::
TestAddColorMaxSizeInvariant::test_history_never_exceeds_max_size fails
under load with hypothesis FailedHealthCheck (too_slow): generating 334
to 400 unique colours per example takes about 0.6 s per ten examples on an
idle machine, and the limit is 1.0 s. This suppresses that one health
check for that one test. Its inputs, its body and its deadline are
unchanged, and the script proves it before writing a byte.
"""
...
SENTINEL = 'RNV-TOO-SLOW'
SENTINEL_FILE = 'tests/test_color_history.py'
#: No new guard file: the round is one statement, and its evidence is the
#: load reproduction in the run-order doc. The file it edits runs first.
GUARD = 'tests/test_color_history.py'
DESCRIPTION = 'suppress hypothesis too_slow on the history max-size property test'
...
LEFT_ALONE = [
    "the test's invariant, its inputs and its body. min_size=334, max_size=400, "
    "the stubbed save and the assertion are exactly as RNV-DEADLINE left them.",
    "the two other property tests in this file, at max_examples=30. They draw "
    "small inputs and keep every health check.",
    "bulk adds in the application: add_colors_batch() and "
    "_add_colors_with_progress() rewrite the whole history file once per "
    "colour. Found 2026-09-12; changing persistence on that path is a "
    "decision, not a fix.",
]

OLD_SETTINGS = '    @settings(max_examples=20, suppress_health_check=[HealthCheck.function_scoped_fixture])\n'


def edits(tree) -> None:
    """One substitution, against the in-memory tree. The anchor is
    checked for exactly one occurrence before anything is written."""
    tree.sub('tests/test_color_history.py',
...
def _test_function(module: ast.Module) -> ast.FunctionDef:
    cls = next(n for n in module.body if isinstance(n, ast.ClassDef)
               and n.name == "TestAddColorMaxSizeInvariant")
    return next(n for n in cls.body if isinstance(n, ast.FunctionDef)
                and n.name == "test_history_never_exceeds_max_size")


def _settings_call(fn: ast.FunctionDef) -> ast.Call:
    return next(d for d in fn.decorator_list if isinstance(d, ast.Call)
                and getattr(d.func, "id", None) == "settings")


def checks(tree) -> None:
    """Against the IN-MEMORY tree, before anything reaches disk."""
    new_src = tree.read('tests/test_color_history.py')
    new = ast.parse(new_src)
    old = ast.parse(_original(tree, 'tests/test_color_history.py'))
    fn_new, fn_old = _test_function(new), _test_function(old)

    # the test itself is untouched: its @given, its body, its signature
    given = lambda fn: [ast.dump(d) for d in fn.decorator_list
                        if isinstance(d, ast.Call) and getattr(d.func, "id", None) == "given"]
    assert given(fn_new) == given(fn_old), "the @given strategy changed"
    assert [ast.dump(s) for s in fn_new.body] == [ast.dump(s) for s in fn_old.body], (
        "the test body changed")
    assert ast.dump(fn_new.args) == ast.dump(fn_old.args), "the signature changed"

    # its @settings: the same examples, one more suppressed check, nothing else
    call = _settings_call(fn_new)
    kw = {k.arg: k.value for k in call.keywords}
    assert set(kw) == {"max_examples", "suppress_health_check"}, (
        f"@settings now sets {sorted(kw)}; a deadline or anything else would "
        f"widen the ruling")
    assert ast.literal_eval(kw["max_examples"]) == 20
    suppressed = [ast.unparse(e) for e in kw["suppress_health_check"].elts]
    assert suppressed == ["HealthCheck.function_scoped_fixture",
                          "HealthCheck.too_slow"], suppressed

    # and nowhere else: too_slow appears in code exactly once, in that call
    uses = [n for n in ast.walk(new) if isinstance(n, ast.Attribute)
            and n.attr == "too_slow"]
    assert len(uses) == 1, f"too_slow is suppressed in {len(uses)} places"

    # every other statement in the file is byte-identical: put the old
    # decorator back and the file must be the file we started from
    start = new_src.index("    # " + SENTINEL)
    end = new_src.index("HealthCheck.too_slow])\n", start) + len("HealthCheck.too_slow])\n")
    assert new_src[:start] + OLD_SETTINGS + new_src[end:] == _original(tree, 'tests/test_color_history.py'), (
        "something outside the decorator changed")
# ------------------------------------------------------------------ plumbing
#
