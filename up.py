...
WHAT THIS REPOSITORY NEEDED.
    ...
    Nothing here was wrong. This repository and the palette manager already
declared >=3.13, already ran 3.13, already capped PyQt6 below 7.0, already
declared pytest-timeout, and already listed 3.13 alone in their classifiers.
They are the shape the other three were moved to.

    the marker        tests/test_brand_contrast.py
    the guard         tests/test_fleet_floor.py

So this script installs the guard and one scanner change, and nothing else.
A round that has nothing to fix in a repository should say so and stop --
the alternative is an edit made to keep five scripts looking alike.
